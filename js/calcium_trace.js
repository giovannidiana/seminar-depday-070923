...
function gentrace(n){
    ...
    while(i<n){
        ...
        if(math.random()<0.2) s=true
        if(s) c1+=50*math.abs(randnScalar())
        c1+=b+randnScalar()*10
...
function display_trace(svgID){
    //console.log("display trace")
    var N=math.round(d3.select("body").node().getBoundingClientRect().width);
    d3.select(svgID).attr("viewBox",[0,0,N,10].join(" "))
    tr = gentrace(N)

    d3.select(svgID).selectAll("line").remove()
